...
class Sphericity(Constraint):
    ...
    def initialize_constraint(self, X, var_idx, vertex_sph, vertices) -> None:
        """ 
        ...
        self.vk_idx = self.V[fix_idx_vertex_sph]
    ...
    def compute(self, X, var_idx):
        ...
        d_vk_E_sph =  2*(vk - cf[self.cf_idx])

        # D_cf E_sph = 2(c_f - v_k)
        d_cf_E_sph = -2*(vk - cf[self.cf_idx])

        # D_rf E_sph = 2(r_f)
        d_rf_E_sph = - 2*rf

        # d_vk
        #self.add_derivatives(self.row_vk, vk_idx, d_vk_E_sph)
