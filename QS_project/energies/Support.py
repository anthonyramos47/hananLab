...
class Supp(Constraint):
    ...
    def __init__(self) -> None:
        ...
        self.line_c = None # Line constraint

      
    def initialize_constraint(self, X, var_idx, sph_sph_adj, line_c) -> None:
        """ 
    ...
    def compute(self, X, var_idx) -> None:
        ...
        for i, f_i in enumerate(self.sph_sph_adj):
            ...
            cij_nd = np.sum((c[edges[:, 1]] - c[edges[:, 0]])*self.line_c[i], axis=1)
            ...
            nd_cjnorm = np.array([self.line_c[i]]).repeat(len(edges), axis=0)/self.cij_norm[i][:,None]
            # d_c(i+1) E_supp_f = nd
            d_cj_E_f  =   nd_cjnorm 
            # d_c(i) E_supp_f = - nd
            d_ci_E_f  =  -nd_cjnorm
