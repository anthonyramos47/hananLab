...
class MeshOptimizer(GuidedProjection):
    _N1 = 0
    ...
    _exclude_free_boundary = False

    _mesh = None

    def __init__(self, vertices=None, halfedges=None):
        ...
        weights = {
            ...
            'forces_minimization': 0}

        self.add_weights(weights)

        if vertices is not None and halfedges is not None:
            self.mesh = Mesh(vertices=vertices, halfedges=halfedges)
    ...
    @property
    def exclude_free_boundary(self):
        return self._exclude_free_boundary

    @exclude_free_boundary.setter
    def exclude_free_boundary(self, bool):
        if bool:
            if not self._exclude_free_boundary:
                self.reinitialize = True
            self._exclude_free_boundary = True
        else:
            if self._exclude_free_boundary:
                self.reinitialize = True
            self._exclude_free_boundary = False
    ...
    def initialize_unknowns_vector(self):
        GuidedProjection.initialize_unknowns_vector(self)
        ...
        if self.get_weight('equilibrium') != 0:
            ...
            # if self.compression:
            #     if self.exclude_free_boundary:
            #         constrained = self.mesh.constrained_vertices
            #         boundary = self.mesh.boundary_vertices()
            #         free = boundary[np.invert(np.in1d(boundary, constrained))]
            #         v1, v2 = self.mesh.edge_vertices()
            #         mask1 = np.in1d(v1, free)
            #         mask2 = np.in1d(v2, free)
            #         mask = np.invert(np.logical_and(mask1, mask2))
            #         W[np.logical_and(mask, W < 0)] = 0
            #     else:
            #         W[W < 0] = 0
            # elif self.tension:
            #     if self.exclude_free_boundary:
    ...
    #     d = geo.sum_repeated(d, f)
    #     r = d / l
    #     e3 = self.mesh.face_normals()
    #     e1 = geo.orthogonal_vectors(e3)
    #     e2 = np.cross(e3, e1)
    ...
    def vector_area_constraints(self):
        w = self.get_weight('area') * self.get_weight('geometric') * 8
        F = self.mesh.F
        N = self.N
        N2 = self._N2
        f = N2 + np.arange(F)
    ...
    def compression_constraints(self):
        ...
        if self.exclude_free_boundary:
            ...
            boundary = self.mesh.boundary_vertices()
            free = boundary[np.invert(np.in1d(boundary, constrained))]
            v = self.mesh.edge_vertices()
            mask1 = np.in1d(v[:, 0], free)
            mask2 = np.in1d(v[:, 1], free)
            mask = np.invert(np.logical_and(mask1, mask2))
            e = e[mask]
        K = len(e)
        i = np.hstack((np.arange(K), np.arange(K)))
        j = np.hstack((N1 + E + e))
        data = np.hstack((np.ones(len(e)))) * w
        H = sparse.coo_matrix((data, (i, j)), shape=(K, N))
        r = np.abs(X[N1 + E + e]) * abs(w)
        self.add_iterative_constraint(H, r, 'compression')
        return H, r

    def forces_minimization(self):
        w = self.get_weight('forces_minimization')
        E = self.mesh.E
        N = self.N
        N1 = self._N1
        X = self.X
        ...
        i = np.hstack((e, e))
        j = np.hstack((d, l))
        data = np.hstack((X[l], X[d])) * w
        K = sparse.coo_matrix((data, (i, j)), shape=(E, N))
        s = X[l] * X[d] * w
        self.add_constant_fairness(K, s, 'forces_minimization')
        return K, s
    ...
    def build_constant_fairness(self):
        ...
        if self.get_weight('forces_minimization') > 0:
            self.forces_minimization()

    def build_iterative_fairness(self):
        pass
        '''
        self.add_weight('N', self.N)
        K, s = self.mesh.iterative_fairness(**self.weights)
        self.add_iterative_fairness(K, s)
        '''
