...
from geolab.plot.imageutilities import blur_shadow, add_shadow, opacity_threshold
...
class Figure(SourcesManager):
    """The figure class.
    Provides functions to generate figures.
    ...
    path : str
        The path of the file to be saved. Default is the current working directory.
    position name : str
        The name of the file containing the position of the figure.
    overwrite : bool (default: True)
        If True, the figure is overwritten if it already exists.
    size : tuple of int (width, height)
        The size of the figure in pixels.
    transparent : bool (default: False)
        If True, the figure background is transparent.
    shade_blur : int
        The blur of the shadow. Default is 20. Higher values correspond to more blurred shadows.
    shade_opacity : float (0 to 1)
        The opacity of the shadow. Default is 0.5.
    """
    ...
    def opacity_threshold(self):
        return self._kwargs.get('opacity_threshold', None)

    @opacity_threshold.setter
    def opacity_threshold(self, threshold):
        self._kwargs['opacity_threshold'] = threshold
    ...
    def save_figure(self):
        ...
        Note:
        -----
            The path is defined by the path attribute.
        """
        self._read_position_file()
        self._make_size()
        self._make_shadow()
    ...
    def rgba(self):
        """Return the rgba array of the figure.

        Returns:
        --------
        rgba : array (n, m, 4)
            The rgba array of the figure.
        """
        self._read_position_file()
        self._make_size()
        self._make_shadow()
    ...
    def make_animation(self, callback, *args, iterations=10, frame_rate=30,
        ...
        """Make an animation and saves the frames as png.

        Parameters
        ----------
        callback : callable
            A function that generates a frame.
        *args :
            The arguments for callback in the same order. The same arguments
            should be returned by the function.

        Optional Parameters
        -------------------
        iterations : int (10)
            The number of iterations for callback.
        frame_rate : int (30)
            The number of frames per second (fps).
        delay : float (0.1)
            The time [s] that each iteration is hold in the animation.
        set_view : bool (True)
            Set the view of the animation.
        """
    ...
    def _rgba(self):
        ...
        img = opacity_threshold(img, self.opacity_threshold)
        return img
